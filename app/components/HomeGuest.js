    ...
    switch (action.type) {
      case "usernameImmediately":
        ...
        if (draft.username.value.length > 30) {
          ...
          draft.username.message = "Choose a username with less than 30 characters.";
        }
        if (draft.username.value && !/^([a-zA-Z0-9]+)$/.test(draft.username.value)) {
          draft.username.hasErrors = true;
          draft.username.message = "Usernames can only contain letters and numbers.";
        }
        return;
      case "usernameAfterDelay":
        if (draft.username.value.length < 3) {
          draft.username.hasErrors = true;
          draft.username.message = "Choose a username with more than 3 characters.";
        }
      ...
      case "passwordImmediately":
        ...
        if (draft.password.value.length > 50) {
          ...
          draft.password.message = "Choose a password with less than 50 characters";
        }
        return;
      case "passwordAfterDelay":
        if (draft.password.value.length < 12) {
          draft.password.hasErrors = true;
          draft.password.message = "Choose a password with at least 12 characters.";
        }
        return;
      case "submitForm":
    ...
    if (state.submitCount) {
      ...
      async function fetchResults() {
        try {
          ...
          appDispatch({ type: "flashMessage", value: "Congrats! Your account has been set up!" });
        } catch (e) {
          console.log("There was a problem or the request was cancelled.");
        }
      }
